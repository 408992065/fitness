...
/**
 * Servlet implementation class UploadVoiceServlet
 */
@WebServlet("/uploadvoice")
public class UploadVoiceServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private IVoiceDao dao =new VoiceDao();
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UploadVoiceServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (isMultipart) {
			...
			File file = new File(getServletContext().getRealPath("/uploadvoice"));
			//end
				...
				while (iter.hasNext()) {
					...
					if (item.isFormField()) {
						...
						if (inputName.equals("coachid")) {
							...
							vc.setCoachid(Integer.parseInt(value));
						}else if(inputName.equals("videoid")){
							vc.setVideoid(Integer.parseInt(value));
						}else if(inputName.equals("userid")) {
							vc.setUserid((Integer.parseInt(value)));
						}else if(inputName.equals("vlength")) {
							vc.setVlength(value);
						}
						...
						vc.setUrl("/uploadvoice/" + newFileName);
						//end
