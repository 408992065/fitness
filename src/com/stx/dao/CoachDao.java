...
public class CoachDao implements ICoachDao{
    ...
    public boolean updateCoach(coach cs) {
        ...
        try {
            con = JDBCUtil.getCon();
            st = con.prepareStatement("update t_coach set name=?,age=?,hight=?,weight=?,gender=?,description=? where coachid=?");
    ...
    public ShowCoach getCoach(int coachid) {
        ...
        try {
            con = JDBCUtil.getCon();
            st = con.prepareStatement("select c.coachid,name,age,gender,hight,weight,url,description from t_coach c,t_jphoto p where c.coachid=? and c.coachid=p.coachid");
            st.setInt(1,coachid);
            ResultSet rs = st.executeQuery();
            if (rs.next()) {
               cr = new ShowCoach(rs.getInt(1),rs.getString(2),rs.getInt(3), rs.getInt(4), rs.getDouble(5), rs.getDouble(6), rs.getString(7),rs.getString(8));
            }
        } catch (ClassNotFoundException | SQLException e) {
            e.printStackTrace();
        } finally {
