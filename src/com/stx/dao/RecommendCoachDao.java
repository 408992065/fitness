	 ...
	 */
	 public ArrayList<RecommendCoach> recommendCoach(int current ,int number) {
		 ArrayList<RecommendCoach> cases = new ArrayList<RecommendCoach>();
	        try {
	            con = JDBCUtil.getCon();
	            st = con.prepareStatement("select* from(select c.coachid,c.name,c.age,c.gender,url,c.price,f.count\r\n" + 
	            		"from (select c.coachid,c.name,c.age,c.gender,p.url,c.price \r\n" + 
	            ...
	            while (rs.next()) {
	            	RecommendCoach sc = new RecommendCoach(rs.getInt(1),rs.getString(2),rs.getInt(3),rs.getInt(4),rs.getString(5),rs.getDouble(6),rs.getInt(7));
	               cases.add(sc);
	            }
